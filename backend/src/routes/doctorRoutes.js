...
const { DOCTOR_STATUSES, DOCTOR_DEPARTMENTS } = require('../constants/enums');
...
const VALID_STATUSES = DOCTOR_STATUSES;
const VALID_DEPARTMENTS = DOCTOR_DEPARTMENTS;
...
const enrich = (doc) => {
  ...
  const workload = doctor.workload ?? Math.round((doctor.patientsToday / doctor.maxPatients) * 100);
  return { ...doctor, workload, isOverloaded: workload > 80 };
};
...
router.get('/', requireAuth, requireHospitalAccess((req) => req.query.hospitalId), asyncHandler(async (req, res) => {
  ...
  const doctors = await Doctor.find(filter).skip((page - 1) * limit).limit(limit).lean();
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const newDoctor = await Doctor.create({
      name: name.trim(), department, hospitalId, status, workload: workloadNum, shift, patientsToday: 0, maxPatients: 20,
    });

    await recordAudit({
      user: req.user, hospitalId, action: 'doctor.create', resourceType: 'doctor', resourceId: newDoctor.id,
      summary: `Added Dr. ${name} (${department})`,
      metadata: { created: { name: name.trim(), department, status, workload: workloadNum, shift } },
    });
    emitToHospital(hospitalId, 'doctor:update', enrich(newDoctor.toJSON()));
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.hospitalId && doctor.hospitalId !== req.user.hospitalId) {
      return res.status(403).json({ success: false, message: 'You can only modify doctors at your own hospital' });
    }
    ...
    await recordAudit({
      user: req.user, hospitalId: doctor.hospitalId, action: 'doctor.update', resourceType: 'doctor', resourceId: safeParamId,
      summary: `Updated Dr. ${doctor.name}${status ? ` → ${status}` : ''}`,
      metadata: {
        before: { status: doctor.status, workload: doctor.workload, shift: doctor.shift },
        after: { status: updated.status, workload: updated.workload, shift: updated.shift },
      },
    });
    emitToHospital(doctor.hospitalId, 'doctor:update', enrich(updated));
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await recordAudit({
      user: req.user, hospitalId: removed.hospitalId, action: 'doctor.delete', resourceType: 'doctor', resourceId: removed.id,
      summary: `Removed Dr. ${removed.name}`,
      metadata: { deleted: { name: removed.name, department: removed.department, status: removed.status } },
    });
    emitToHospital(removed.hospitalId, 'doctor:delete', { id: removed.id });
